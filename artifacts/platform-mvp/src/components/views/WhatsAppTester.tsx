...
export default function WhatsAppTester({
  ...
  const [provider, setProvider] = useState<Provider>(
    business.whatsapp_provider === "emovur" ? "emovur" : "gupshup"
  );
  ...
  // Emovur config
  const [emovurUrl, setEmovurUrl] = useState("");
  const [savingEmovur, setSavingEmovur] = useState(false);
  const emovurReady = business.whatsapp_provider === "emovur";
  ...
  useEffect(() => {
    ...
    setProvider(business.whatsapp_provider === "emovur" ? "emovur" : "gupshup");
  }, [business]);
  ...
  const settingsComplete = !!sourceNumber.trim() && !!appName.trim() && !!appId.trim();

  async function saveSettings() {
    setSavingSettings(true);
    setResult(null);
    try {
      await api.post("/integrations/whatsapp/settings", { sourceNumber, appName, appId });
      onSaved();
  ...
  async function saveEmovur() {
    if (!emovurUrl.trim()) return;
    setSavingEmovur(true);
    setResult(null);
    try {
      await api.post("/integrations/whatsapp/emovur/settings", { templateUrl: emovurUrl.trim() });
      onSaved();
      setEmovurUrl("");
      setResult({ type: "success", msg: "Emovur template connected. You can send a test below." });
    } catch (err: any) {
      setResult({
        type: "error",
        msg: err?.response?.data?.error || "Couldn't save your Emovur link. Please try again.",
      });
    } finally {
      setSavingEmovur(false);
    }
  }
    ...
    try {
      ...
      setResult({
        ...
        msg:
          ...
          "Couldn't load your templates. Check your App ID and API key.",
      });
    } finally {
  ...
  async function sendTest() {
    ...
    try {
      if (provider === "emovur") {
        const { data } = await api.post("/integrations/whatsapp/emovur/test", { destination });
        setResult({
          type: "success",
          msg: `Sent to ${destination} — Emovur accepted it${
  ...
  const canSend =
    !!destination.trim() && (provider === "emovur" ? emovurReady : !!templateId);

  function tabCls(p: Provider) {
    return `flex-1 text-xs font-semibold py-2 rounded-lg transition-colors ${
  ...
  return (
    <div className="mb-8 bg-white rounded-2xl border border-emerald-200 px-5 py-5">
      ...
      <p className="text-xs text-gray-500 mb-4 leading-relaxed">
        Send a real WhatsApp message to make sure everything works. Pick the provider your number is
        connected through.
      </p>

      {/* Provider toggle */}
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 mb-5">
        <button onClick={() => setProvider("gupshup")} className={tabCls("gupshup")}>
          Gupshup
        </button>
        <button onClick={() => setProvider("emovur")} className={tabCls("emovur")}>
          Emovur
        </button>
      </div>
          ...
          <div className="space-y-3 mb-5">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={savingSettings || !settingsComplete}
        ...
        <>
          ...
          <div className="space-y-3 mb-5">
            <div>
              ...
              <input
                type="password"
                value={emovurUrl}
                onChange={(e) => setEmovurUrl(e.target.value)}
                placeholder={
                  emovurReady
                    ? "Saved — paste a new link to replace it"
                    : "https://adminapis.backendprod.com/.../template/.../process"
                }
                className={inputCls}
              />
              <p className="text-[11px] text-gray-400 mt-1.5 leading-relaxed">
                From Emovur → WhatsApp → Templates → your template → “View/Edit Webhook details” →
                copy the API URL. Saved securely and never shown again.
              </p>
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={saveEmovur}
                disabled={savingEmovur || !emovurUrl.trim()}
                className="bg-gray-900 text-white text-xs font-semibold px-4 py-2 rounded-xl hover:bg-gray-700 disabled:opacity-40 transition-colors"
              >
                {savingEmovur ? "Saving..." : emovurReady ? "Replace link" : "Connect Emovur"}
              </button>
              {emovurReady && (
          ...
          <div className="border-t border-gray-100 pt-5">
            <p className="text-xs text-gray-400 mb-1">
              {emovurReady
                ? "Sends your connected Emovur template to the number below."
                : "Connect your Emovur template above, then pick a number to test."}
            </p>
          </div>
        </>
      )}

      {/* Destination + send (shared) */}
